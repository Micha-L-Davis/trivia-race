...
// Sets current question to question spliced from question list. Refills the question list if empty.
function chooseQuestion() {
  let index = randomIndex(questionList);
...
// Event handler for feedback message confirmation
...
// Player starting choices event handler
...
// Find out if player's guess is good, add to weighted list, inform player of result
function judgePlayerGuess(targetValue) {
  totalQuestions++;
  if (targetValue === currentQuestion.answer) {
...
// Flips a coin to see if the AI guesses correctly
function judgeAIGuess() {
...
// Sets racer object's coordinate properties to the position that matches their current score
...
// Renders racer images at the object's coordinate property value
...
// Notify player of win or loss and tallies final score
function endRace() {
...
// Make sure we have a LB array
let leaderboard = [];
// Retrieves any existing leaderboard data, appends recent playthrough data, and saves to local storage
...
// Returns a random int inclusive of the max bounds
function randomIntInclusive(min, max) {
  return Math.floor(Math.random() * (max - min + 1) + min);
}

// Returns a random index from the supplied array
function randomIndex(arr) {
  return Math.floor(Math.random() * arr.length);
}

// Save player object into local storage
function storeData() {
  console.log('saving');
